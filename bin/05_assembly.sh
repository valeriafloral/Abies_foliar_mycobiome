...
for f in `ls ../data/filter/outputs/ | grep ".fq.gz" | sed "s/_L007_R1_001_paired.fq.gz//"| sed "s/_L007_R2_001_paired.fq.gz//" | sed "s/_L007_R1_001_unpaired.fq.gz//"| sed "s/_L007_R2_001_unpaired.fq.gz//"| uniq`;
do spades.py --rna --meta -1 ../data/filter/outputs/${f}_R1_unpaired_bulk.fastq -2 ../data/filter/outputs/${f}_R1_unpaired_bulk.fastq --s1 ../data/filter/outputs/${f}_R1_unpaired_bulk.fastq --s2 ../data/filter/outputs/${f}_R2_unpaired_bulk.fastq -t 4 -k 21,33,55,77,99,111,127 -o ../data/reports/assembly/${f}_assembly;
done
